...
this.onPhoneEvent = function(event) {
  ...
  if (event && event.name === "ASR.Result") {
    ...
    if (recognizedText && recognizedText.length > 0 && confidence > 50 && !isProcessing) {
      ...
      // Останавливаем ASR перед отправкой в AI
      if (currentASR) {
        currentASR.stop();
        currentASR = null;
      }
  ...
  // Передаём оригинальному обработчику
...
function getAIResponse(userText) {
  ...
  Net.httpRequestAsync(webhookUrl, {
    ...
    // ✅ ИСПРАВЛЕНИЕ: Сначала говорим текст, ЗАТЕМ проверяем action
    currentCall.say(aiResponse, Language.RU_RUSSIAN_MALE);
    
    // Проверка команды перевода на оператора
    if (action === "transfer" && transferPhone) {
      Logger.write("[" + tenant_slug + "] TRANSFER REQUEST to: " + transferPhone);
      
      // Регистрируем одноразовый обработчик для перевода ПОСЛЕ текущего say()
      var transferHandler = function() {
        Logger.write("[" + tenant_slug + "] Starting call transfer to: " + transferPhone);
        ...
        var adminCall = VoxEngine.callPSTN(transferPhone, currentCall.callerid());
        
        adminCall.addEventListener(CallEvents.Connected, function() {
          Logger.write("[" + tenant_slug + "] Admin connected, bridging calls");
          VoxEngine.sendMediaBetween(currentCall, adminCall);
        });
        
        adminCall.addEventListener(CallEvents.Failed, function(ev) {
          Logger.write("[" + tenant_slug + "] Transfer failed: " + ev.reason);
          currentCall.say("Извините, не удалось соединить с оператором. Попробуйте позже.", Language.RU_RUSSIAN_MALE);
          currentCall.hangup();
        });
        
        adminCall.addEventListener(CallEvents.Disconnected, function() {
          Logger.write("[" + tenant_slug + "] Admin disconnected");
          currentCall.hangup();
        });
      };
      
      // Добавляем обработчик для перевода
      currentCall.addEventListener(CallEvents.PlaybackFinished, transferHandler);
      
    } else {
      // Обычный AI-ответ — после воспроизведения возвращаемся к прослушиванию
      isProcessing = false;
    }
...
// Функция удаления эмодзи из текста
function removeEmojis(text) {
  return text.replace(/[\u{1F600}-\u{1F64F}]/gu, '')  // Эмоции
    .replace(/[\u{1F300}-\u{1F5FF}]/gu, '')  // Символы и пиктограммы
    .replace(/[\u{1F680}-\u{1F6FF}]/gu, '')  // Транспорт
